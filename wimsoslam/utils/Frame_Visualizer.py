...
from wimsoslam.common import cam_pose_to_matrix
...
class Frame_Visualizer(object):
    ...
    def save_imgs(self, idx, iter, frame_dict, feature_maps, gt_depth, gt_color, c2w_or_camera_tensor, all_planes, decoders):
        """
        ...
        with torch.no_grad():
            if (idx % self.freq == 0) and (iter % self.inside_freq == 0):
                ...
                depth, d_var, uncertainty, color = self.renderer.render_img(all_planes, decoders, c2w, self.truncation, frame_dict, feature_maps,
                ...
                axs[2, 0].set_xticks([])
                axs[2, 0].set_yticks([])
                axs[2, 1].imshow(np.sqrt(uncertainty_np), cmap="jet", vmin=0, vmax=0.2)
                axs[2, 1].set_title(f"Epistemic: {np.sqrt(uncertainty_np).max():.4f}")
                axs[2, 1].set_xticks([])
                axs[2, 1].set_yticks([])
                axs[2, 1].imshow(d_var_np[..., 0], cmap="jet", vmin=0, vmax=0.2)
                axs[2, 2].set_title(f"Aleatoric 1: {d_var_np[..., 0].max():.4f}")
                axs[2, 2].set_xticks([])
                axs[2, 2].set_yticks([])
